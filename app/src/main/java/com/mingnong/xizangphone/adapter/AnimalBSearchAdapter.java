 ...
 */
public class AnimalBSearchAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private List<QueryAnimBBean.DataListBean> list = new ArrayList<>();
    ...
    public AnimalBSearchAdapter(Context context, List<QueryAnimBBean.DataListBean> list, View headerView) {
    ...
    public List<QueryAnimBBean.DataListBean> getDates() {
        return list;
    }

    public void setmDatas(List<QueryAnimBBean.DataListBean> list) {
        if (this.list == null) {
            this.list = list;
        } else {
            this.list.clear();
    ...
    public void addAll(List<QueryAnimBBean.DataListBean> list) {
        if (this.list == null) {
            this.list = list;
        } else {
            this.list.addAll(list);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ListviewViewHolder) {
            ((ListviewViewHolder) holder).tvHuozhuName.setText(list.get(position - 1).getAQCARGOOWNER());
            ((ListviewViewHolder) holder).tvShenbaoPhone.setText(list.get(position - 1).getAQPHONE());
            ((ListviewViewHolder) holder).tvAnimalType.setText(list.get(position - 1).getAQXUZHONG());
            ((ListviewViewHolder) holder).tvAnimalNumber.setText(list.get(position - 1).getAQQUANTITY() + "");
        }
    }
