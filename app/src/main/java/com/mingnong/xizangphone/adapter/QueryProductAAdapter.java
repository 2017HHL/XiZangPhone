...
public class QueryProductAAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public QueryProductAAdapter(Context context, List<QueryProductABean.DataListBean> list, View headerView) {
...
//            ((ListviewViewHolder) holder).tvHarmlseeType.setText(list.get(position - 1).getIsPrant());
            ...
            ((ListviewViewHolder) holder).tvJianyiBianhao.setText(list.get(position - 1).getPNUMBER());//检疫证编号
            ((ListviewViewHolder) holder).tvHouzhuName.setText(list.get(position - 1).getPCARGOOWNER());//货主
            ((ListviewViewHolder) holder).tvAnimalType.setText(list.get(position - 1).getPUNITNAME());//生产单位名称
            ((ListviewViewHolder) holder).tvQuantityDanwei.setText(list.get(position - 1).getPQUANTITY() + list.get(position - 1).getPDANWEI());//数量及单位
        }
    }
    ...
    public int getItemViewType(int position) {
        if (position == 0) {
            return QueryProductAAdapter.ITEM_TYPE.ITEM_HEAD.ordinal();
        } else {
            return QueryProductAAdapter.ITEM_TYPE.ITEM_LISTVIEM.ordinal();
        }
    }
