 ...
 */
public class AnimalBActivityPresenter extends BaseActivityPresenter<IAnimalBActivity> {
    public AnimalBActivityPresenter(IAnimalBActivity mView) {
        super(mView);
    }

    public void upload(AnimBUploadBean data) {
        ...
        if (NetWorkUtils.isOpenNetwork(MyApplication.getContext())) {
            addSubscribe(NetClient.upload(userId, "AMB", new Gson().toJson(data))
            ...
            PrintDB.getInstance(MyApplication.getContext()).insert(getUserId(), "AMB", new Gson().toJson(data));
            getView().uploadsuccess();
        }
    }

    public void getJianyiZhenghao() {
        addSubscribe(NetClient.getJianyiZhenghao(getUserId(), "AMB")
                .map(baseMsgBean -> {
