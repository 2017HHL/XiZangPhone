 ...
 */
public class ProductAActivityPresenter extends BaseActivityPresenter<IProductAActivity> {
    public ProductAActivityPresenter(IProductAActivity view) {
        super(view);
    }

    public void upLoad(ProductAUploadBean data) {
        ...
        if (NetWorkUtils.isOpenNetwork(MyApplication.getContext())) {
            addSubscribe(NetClient.upload(userId, "PDA", new Gson().toJson(data))
            ...
            PrintDB.getInstance(MyApplication.getContext()).insert(getUserId(), "PDA", new Gson().toJson(data));
            getView().upLoadSucceed();
        }
    }

    public void getJianyiZhenghao() {
        addSubscribe(NetClient.getJianyiZhenghao(getUserId(), "PDA")
                .map(baseMsgBean -> {
