    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        ...
        if (ex == null && mDefaultCrashHandler != null) {
            ...
            if (NetWorkUtils.isOpenNetwork(mContext)) {
                ...
                //清空外部数据库
                File outterDb = FileUtil.getInstance().getDbFile();
                if (outterDb.exists()) {
                    outterDb.delete();
                }
                //清空内部数据库
                File innerDb = FileUtil.getInstance().getDbFile();
                if (innerDb.exists()) {
                    innerDb.delete();
                }
                //清空sp
                SPUtils.getInstance().removeKey(Contance.SP_DB_VERSION);
                SPUtils.getInstance().removeKey(Contance.SP_PATCH_VERSION);
            }
