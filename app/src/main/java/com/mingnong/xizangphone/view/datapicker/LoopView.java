package com.mingnong.xizangphone.view.datapicker;
...
public class LoopView extends View {
    ...
    private void e() {
        ...
        for (int i1 = 0; i1 < arrayList.size(); i1++) {
            ...
            if (arrayList.get(i1).length() >= length) {
                length = arrayList.get(i1).length();
                paintB.getTextBounds(arrayList.get(i1), 0, arrayList.get(i1).length(), rect);
            }
            int j1 = rect.width();
    ...
    protected final void c() {
        if (loopListener != null) {
            (new Handler()).postDelayed(new LoopRunnable(this), 50L);
        }
    }
